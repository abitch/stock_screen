...
- **交互式菜单**:运行即进入,默认展示自选股行情(东方财富风格:最新价/涨跌额/涨跌幅,涨红跌绿)
- **自选股管理**:搜索股票、收藏到自选、移除;搜索结果支持分页浏览
- **快速行情**:自选股行情用新浪轻量接口按代码精确查询,几只股票约 0.2s 返回,不再逐只算均线
- **股票列表本地缓存**:全市场代码/名称持久化到 SQLite,启动优先读缓存(默认 7 天有效),搜索秒出
- 全市场 A 股筛选,找出收盘价在 MA20 之上的股票(均线判断按需手动触发)
...
直接运行,进入交互界面,默认展示自选股行情(最新价/涨跌额/涨跌幅):
...
| 选项 | 功能 |
|------|------|
| `0` | 刷新自选股行情(联网拉最新价/涨跌额/涨跌幅) |
| `1` | 搜索股票并收藏到自选(按代码或名称,结果分页,`n`/`p` 翻页) |
| `2` | 从自选移除 |
| `3` | 筛选自选股中站上 MA20 的 |
...
> 启动页只展示行情,不计算均线;MA20 站上/跌破判断在菜单 `3`/`4` 按需触发。
...
> 股票列表始终通过 akshare 的 `stock_info_a_code_name` 等轻量接口获取(带多接口自动回退),并持久化到 SQLite 的 `stocks` 表;启动后优先读本地缓存(默认 7 天有效,可在 `config.yaml` 的 `data.stock_list_ttl_days` 调整),避免每次联网拉全市场。

> 自选股实时行情用新浪轻量接口 `hq.sinajs.cn` 按代码精确批量查询,只取自选股,不拉全市场快照,因此刷新很快;失败时回退东方财富全市场快照。
